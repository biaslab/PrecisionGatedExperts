...
const RESULTS_DIR = joinpath(@__DIR__, "..", "paper", "results_vae_std")
...
const TEST_SIZE_BY_DATASET = Dict(
    "ETTh1" => 2881,
    "ETTh2" => 2881,
    "exchange_rate" => 1422,
    "electricity" => 5261,
...
const SCI_NOTATION_THRESHOLD = 1000.0
const SCI_DIGIT_COUNT_THRESHOLD = 6
...
function dataset_label_for_table(name::String)
    return name == "Exchange rate" ? "\\shortstack{Exchange\\\\rate}" : name
end
...
function load_metrics(dataset::String, horizon::Int, pred_type::Symbol, model_type::String)
    ...
    return Dict(
        ...
        :mse_std => clean_metric(get_metric_value(metrics, :mse_std)),
        :mse_std_mv => clean_metric(get_metric_value(metrics, :mse_std_mv)),
        :mae => clean_metric(get_metric_value(metrics, :mae)),
        :nll => clean_metric(negate_if_present(get_metric_value(metrics, :nll))),
        :nll_std => clean_metric(get_metric_value(metrics, :nll_std)),
...
function fmt(val; force_decimals::Union{Nothing,Int}=nothing)
    if val === nothing
        return "\$\\times\$"
    end
    abs_val = abs(val)
    if abs_val == 0.0
        d = force_decimals === nothing ? 3 : force_decimals
        return @sprintf("%.*f", d, 0.0)
    elseif abs_val >= SCI_NOTATION_THRESHOLD
        exp = floor(Int, log10(abs_val))
        coeff = val / 10.0^exp
        d = force_decimals === nothing ? 3 : force_decimals
        return "\$" * @sprintf("%.*f", d, coeff) * "\\!\\times\\!10^{$exp}\$"
    elseif force_decimals !== nothing
        return @sprintf("%.*f", force_decimals, val)
...
function mean_decimals_for_metric(metric_key::Union{Nothing,Symbol})
    metric_key == :nll && return 1
    metric_key == :mse && return 3
    return nothing
end

function std_decimals_for_metric(metric_key::Union{Nothing,Symbol})
    metric_key == :nll && return 1
    metric_key == :mse && return 2
    return nothing
end
...
function highlight_metric(val, best_val, second_val; metric_key::Union{Nothing,Symbol}=nothing)
    decimals = mean_decimals_for_metric(metric_key)
    rendered = fmt(val; force_decimals=decimals)
...
function sci_parts(val)
    val === nothing && return nothing
    abs_val = abs(val)
    abs_val >= SCI_NOTATION_THRESHOLD || return nothing
    exp = floor(Int, log10(abs_val))
    coeff = val / 10.0^exp
    return (coeff=coeff, exp=exp)
end

function fixed_digit_count(val, decimals::Int)
    s = @sprintf("%.*f", decimals, abs(val))
    return count(isdigit, s)
end

function fmt_with_std(val, std_val; metric_key::Union{Nothing,Symbol}=nothing)
    mean_decimals = mean_decimals_for_metric(metric_key)
    std_decimals = std_decimals_for_metric(metric_key)
    std_val === nothing && return fmt(val; force_decimals=mean_decimals)

    # For non-scientific NLL, use finer precision.
    if metric_key == :nll
        mean_decimals = 3
        std_decimals = 2
    end
    use_scientific = val !== nothing && (
        abs(val) >= SCI_NOTATION_THRESHOLD ||
        abs(std_val) >= SCI_NOTATION_THRESHOLD ||
        fixed_digit_count(val, mean_decimals === nothing ? 3 : mean_decimals) >= SCI_DIGIT_COUNT_THRESHOLD ||
        fixed_digit_count(std_val, std_decimals === nothing ? 3 : std_decimals) >= SCI_DIGIT_COUNT_THRESHOLD
    )
    base = fmt(val; force_decimals=mean_decimals)

    # Use one shared scientific multiplier whenever either part is large.
    if use_scientific
        exp_candidates = Int[]
        if abs(val) > 0.0
            push!(exp_candidates, floor(Int, log10(abs(val))))
        end
        if abs(std_val) > 0.0
            push!(exp_candidates, floor(Int, log10(abs(std_val))))
        end
        if !isempty(exp_candidates)
            shared_exp = maximum(exp_candidates)
            d_mean = 1
            d_std = 1
            coeff_val = val / 10.0^shared_exp
            coeff_std = std_val / 10.0^shared_exp
            return "\$(" * @sprintf("%.*f", d_mean, coeff_val) * " \\pm " * @sprintf("%.*f", d_std, coeff_std) * ")\\!\\times\\!10^{$shared_exp}\$"
        end
    end
    return "$base \\(\\pm\\) $(fmt(std_val; force_decimals = std_decimals))"
end

function highlight_metric_with_std(val, std_val, best_val, second_val; metric_key::Union{Nothing,Symbol}=nothing)
    rendered = fmt_with_std(val, std_val; metric_key=metric_key)
    if val !== nothing && best_val !== nothing && val == best_val
        return "\\textbf{$rendered}"
    elseif val !== nothing && second_val !== nothing && val == second_val
        return "\\underline{\\textcolor{blue}{$rendered}}"
...
function mse_std_key_for_dataset(ds::String)
    return ds in ("ETTh1", "ETTh2") ? :mse_std : :mse_std_mv
end

function ci_half_width(ds::String, std_val)
    std_val === nothing && return nothing
    n = get(TEST_SIZE_BY_DATASET, ds, nothing)
    n === nothing && return nothing
    return Z_SCORE_95 * std_val / sqrt(n)
end
...
function build_latex_table()
    ...
    for (di, (ds, display_name, _)) in enumerate(DATASETS)
        ...
        for (hi, horizon_label) in enumerate(horizon_groups)
            ...
            per_model_std_metrics = Dict{String,metric_dict_type}()
            for mt in ENSEMBLE_MODEL_TYPES
                if horizon_label == "Avg"
                    ...
                    per_model_std_metrics[mt] = Dict(
                        :mse => let std_key = mse_std_key_for_dataset(ds)
                            vals = [m[std_key] for m in model_metrics if get(m, std_key, nothing) !== nothing]
                            isempty(vals) ? nothing : sum(vals) / length(vals)
                        end,
                        :nll => let vals = [m[:nll_std] for m in model_metrics if get(m, :nll_std, nothing) !== nothing]
                            isempty(vals) ? nothing : sum(vals) / length(vals)
                        end,
                    )
                else
                    ...
                    if metrics === nothing
                        per_model_std_metrics[mt] = Dict(:mse => nothing, :nll => nothing)
                    else
                        per_model_std_metrics[mt] = Dict(
                            :mse => get(metrics, mse_std_key_for_dataset(ds), nothing),
                            :nll => get(metrics, :nll_std, nothing),
                        )
                    end
                end
            end
            if horizon_label == "Avg"
                ...
                per_model_std_metrics["best_baseline"] = Dict(:mse => nothing, :nll => nothing)
            else
                h = parse(Int, horizon_label)
            ...
            for (mi, metric) in enumerate(metric_names)
                ...
                dataset_cell = (hi == 1 && mi == 1) ? "\\multirow{$dataset_row_span}{*}{{\\scriptsize $(dataset_label_for_table(display_name))}}" : ""
                horizon_cell = mi == 1 ? "\\multirow{$(length(metric_names))}{*}{$horizon_label}" : ""

                row_cells = String[]
                for mt in table_models
                    model_metrics = per_model_metrics[mt]
                    if metric.key in (:mse, :nll)
                        std_val = per_model_std_metrics[mt][metric.key]
                        ci_val = ci_half_width(ds, std_val)
                        push!(
                            row_cells,
                            highlight_metric_with_std(
                                model_metrics[metric.key],
                                ci_val,
                                best_val,
                                second_val,
                        ...
                        push!(row_cells, highlight_metric(model_metrics[metric.key], best_val, second_val; metric_key=metric.key))
                    end
                end
