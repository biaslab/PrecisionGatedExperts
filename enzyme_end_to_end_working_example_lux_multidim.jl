...
function create_dataset(; dataset_size=1000, sequence_length=50)
    ...
    # Get the labels
    labels = vcat(zeros(Float32, dataset_size ÷ 2, 2), ones(Float32, dataset_size ÷ 2, 2))'
    clockwise_spirals = [
...
function SpiralClassifier(in_dims, hidden_dims, out_dims)
    return SpiralClassifier(
        LSTMCell(in_dims => hidden_dims), Dense(hidden_dims => out_dims, identity)
    )
end
