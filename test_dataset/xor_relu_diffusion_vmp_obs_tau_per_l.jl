...
const OUTPUT_PREFIX = get(ENV, "OUTPUT_PREFIX", "test_dataset/viz/xor_relu_diffusion_vmp_obs_tau_per_l")
...
const SCHEDULE = lowercase(get(ENV, "SCHEDULE", "linear"))
...
# Per-level obs_tau prior shape. Default 2.0 = loose (std = mean/sqrt(shape)).
# Rate is computed per level to set mean = 1/sigma^2[l] (schedule-informed).
# To pin obs_tau close to schedule (reference-style tight prior), set OBS_TAU_SHAPE=1e8.
const OBS_TAU_SHAPE = parse(Float64, get(ENV, "OBS_TAU_SHAPE", "2.0"))
...
# ---------------------------------------------------------------------------
# Diffusion model — per-level learned obs_tau[l].
# Weights w_mean[k], w_a[k] are shared across levels (weight tying, same as v1).
# Chain: out[l, j] ~ N(out[l-1, j], chain_var).
# Noise-end anchor: out[1, j] ~ N(0, sigma^2[1]).
...
@model function xor_relu_diffusion_vmp(n_neurons, n_levels, features, y, priors,
    ...
    for j = 1:size(y, 2)
        ...
        for l = 1:n_levels
            ...
            y[l, j] ~ NormalMeanPrecision(out[l, j], obs_tau[l])
        end
    end
end
...
@model function xor_relu_diffusion_vmp_prediction(n_neurons, n_levels, n_obs, features, priors,
    ...
    for j = 1:n_obs
        ...
        for l = 1:n_levels
            ...
            y[l, j] ~ NormalMeanPrecision(out[l, j], obs_tau[l]) where {meta = StrangeMissingMeta()}
            y[l, j] ~ Uninformative()
        end
    end
end
...
@constraints function xor_relu_diffusion_vmp_prediction_constraints(priors)
    ...
    for (l, prior) in enumerate(priors[:obs_tau])
        q(obs_tau[l])::RxInfer.FixedMarginalFormConstraint(prior)
    end
...
@initialization function xor_relu_diffusion_vmp_init(priors)
    ...
    q(τ)       = priors[:τ]
    q(obs_tau) = deepcopy(priors[:obs_tau])
...
# ---------------------------------------------------------------------------
# Schedule & target generation (same as v1)
# ---------------------------------------------------------------------------
...
# Per-level obs_tau prior: Gamma(shape, shape*sigma^2[l]) so E[obs_tau[l]] = 1/sigma^2[l].
# shape controls tightness (small = loose/learnable, large = pinned to schedule).
function make_obs_tau_priors(sigma2_sched; shape = OBS_TAU_SHAPE)
    return [GammaShapeRate(shape, shape * s) for s in sigma2_sched]
end

function make_priors(; sigma2_sched, n_neurons = N_NEURONS, n_features = 3, seed = PRIOR_SEED)
    rng = StableRNG(seed)
    return Dict{Symbol,Any}(
        ...
        :obs_tau => make_obs_tau_priors(sigma2_sched),
    )
end
...
function posterior_priors(result; iteration = length(result.posteriors[:w_mean]))
    return Dict{Symbol,Any}(
        ...
        :obs_tau => deepcopy(result.posteriors[:obs_tau][iteration]),
    )
end
...
function obs_tau_means(result, iteration)
    return [Float64(mean(result.posteriors[:obs_tau][iteration][l])) for l in 1:L_DEPTH]
end
...
function training_diagnostics(result, y_true_train)
    ...
    obs_tau_cols = [Symbol("obs_tau_l$l") for l in 1:L_DEPTH]
    for c in obs_tau_cols
        rows[!, c] = Float64[]
    end
    ...
    for iteration in 1:n_it
        ...
        obs_taus = obs_tau_means(result, iteration)
        base = (
            iteration,
            fe_val,
...
function save_free_energy_plot(diagnostics)
    ...
    p = plot(
        ...
        xlabel = "iteration",
        ylabel = "free energy",
        title = "XOR diffusion VMP per-level obs_tau (L=$L_DEPTH, $SCHEDULE)",
        label = "free energy",
        size = (760, 440),
    )
    savefig(p, outpath)
...
function save_stability_plot(diagnostics)
    ...
    p1 = plot(diagnostics.iteration, diagnostics.train_mse_cleanend;
        ...
        title = "Train MSE", label = "train MSE", lw = 2)

    p2 = plot(xlabel = "iteration", ylabel = "E[obs_tau[l]]",
              title = "Per-level obs_tau", yscale = :log10, lw = 2)
    for l in 1:L_DEPTH
        col = Symbol("obs_tau_l$l")
        plot!(p2, diagnostics.iteration, diagnostics[!, col];
            marker = :circle, label = "l=$l", lw = 2)
    end

    p3 = plot(diagnostics.iteration, diagnostics.w_mean_max_norm;
        marker = :circle, xlabel = "iteration", ylabel = "max |w|",
        title = "Weight norms", label = "|w_mean|", lw = 2)
    plot!(p3, diagnostics.iteration, diagnostics.w_a_max_norm;
        marker = :square, label = "|w_a|", lw = 2)

    plot(p1, p2, p3; layout = (1, 3), size = (1300, 360),
        plot_title = "diffusion VMP stability — per-level obs_tau (L=$L_DEPTH, $SCHEDULE)")
    savefig(outpath)
    return outpath
end
...
function save_prediction_csv(marginals, df_test)
    ...
    predictions = DataFrame(
        x1 = df_test.x1, x2 = df_test.x2, y_true = df_test.OT,
        y_mean = y_mean, y_variance = y_var, y_std = sqrt.(max.(y_var, 0.0)),
    )
...
function build_iteration_gif(training_result, grid_x, grid_y, features_grid, df_test,
    ...
    println("\nBuilding heatmap GIF over $(length(iter_indices)) VMP iterations")
    ...
    for (f, it) in enumerate(iter_indices)
        ...
        pr = Dict{Symbol,Any}(
            ...
            :obs_tau => deepcopy(training_result.posteriors[:obs_tau][it]),
        )
        ...
        plot(p1, p2; layout = (1, 2), size = (900, 400),
            plot_title = "per-level obs_tau  —  VMP iter $it/$n_it  (L=$L_DEPTH, $SCHEDULE)")
        frame(anim)
    end
...
function save_prediction_heatmaps(grid_x, grid_y, grid_marginals, actual_grid, df_test)
    ...
    plot(p1, p2, p3; layout = (1, 3), size = (1350, 420),
        plot_title = "per-level obs_tau (L=$L_DEPTH, $SCHEDULE)")
    savefig(outpath)
    return outpath
end
...
priors = make_priors(sigma2_sched = sigma2_sched)

prior_var = sigma2_sched[1]
...
println("=" ^ 78)
println("XOR diffusion VMP (weight-tied, PER-LEVEL learned obs_tau)")
...
println("  obs_tau prior shape = $OBS_TAU_SHAPE (small = loose)")
println("  obs_tau prior means  = $(round.([1.0/s for s in sigma2_sched], digits=3))")
...
@printf("Final τ = %.6f\n", diagnostics.tau[end])
println("Final obs_tau per level = ",
        join([@sprintf("l%d=%.3f", l, diagnostics[end, Symbol("obs_tau_l$l")]) for l in 1:L_DEPTH], ", "))
...
for row in eachrow(diagnostics)
    obs_tau_str = join([@sprintf("%.2e", row[Symbol("obs_tau_l$l")]) for l in 1:L_DEPTH], ",")
    @printf("  iter %2d: FE=%.3e  τ=%.2e  obs_tau=[%s]  |w_m|=%.3f  |w_a|=%.3f  train_mse(L)=%.5f\n",
        row.iteration, row.free_energy, row.tau, obs_tau_str,
