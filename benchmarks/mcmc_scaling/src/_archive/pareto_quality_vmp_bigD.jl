# Extend VMP (both 10-iter and per-N converged) sweeps to D ∈ [25, 65] at N ∈ [10, 50].
...
const CSV10 = joinpath(RESULTS_DIR, "pareto_quality_vmp.csv")
const CSVC  = joinpath(RESULTS_DIR, "pareto_quality_vmp_converged.csv")

const N_EXPERTS = 7
iters_converged(N::Int) = N ≤ 10 ? 20 : N ≤ 50 ? 50 : 100

const NEW_CONFIGS = [(10, 25), (50, 25), (10, 65), (50, 65)]

function append_row_vmp10!(row)
    schema = DataFrame(
    ...
    push!(schema, row)
    CSV.write(CSV10, schema; append=true)
end
function append_row_conv!(row)
    schema = DataFrame(
        method=String[], N=Int[], D=Int[],
        rel_err_total=Float64[], rel_err_w=Float64[], rel_err_τ=Float64[], rel_err_β=Float64[],
        cov95=Float64[], cov50=Float64[],
    ...
    push!(schema, row)
    CSV.write(CSVC, schema; append=true)
end
...
function run_vmp_cfg(N::Int, D::Int, iters::Int; seed=42)
...
println("=" ^ 70)
println("Extending VMP sweeps to big D: $NEW_CONFIGS")
println("=" ^ 70)
...
for (N, D) in NEW_CONFIGS
    println("\n[N=$N, D=$D]")
    # 10-iter (paper default)
    print("  VMP-10 iters...")
    r = run_vmp_cfg(N, D, 10)
    println(" err=$(round(r.rel_err, digits=3))  cov95=$(round(r.cov95, digits=2))  t=$(round(r.total_time_s, digits=2))s  $(r.status)")
    append_row_vmp10!(("VMP-dynamic", N, D, r.rel_err, r.rel_err_w, r.rel_err_τ, r.rel_err_β,
                        r.cov95, r.cov50, r.std_err, r.total_time_s, r.free_energy, r.status))

    # Converged (per-N iters)
    iters = iters_converged(N)
    print("  VMP-conv ($iters iters)...")
    r = run_vmp_cfg(N, D, iters)
    println(" err=$(round(r.rel_err, digits=3))  cov95=$(round(r.cov95, digits=2))  t=$(round(r.total_time_s, digits=2))s  $(r.status)")
    append_row_conv!(("VMP-converged", N, D, r.rel_err, r.rel_err_w, r.rel_err_τ, r.rel_err_β,
                      r.cov95, r.cov50, r.std_err, r.total_time_s, r.free_energy, iters, r.status))
end

println("\nAppended to both VMP CSVs")
