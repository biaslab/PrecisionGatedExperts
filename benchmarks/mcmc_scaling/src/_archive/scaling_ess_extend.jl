...
# Reuse all constants from the small sweep (priors, budgets, GH nodes)
const N_EXPERTS = 7
const K_GH = 15
...
# Only the new N values to extend the N@D2 sweep
const N_EXTEND = [250, 500]

# ==================================================================
# Runners (identical to scaling_ess_small)
# ==================================================================
...
# ==================================================================
# Run — extend N@D2 sweep
# ==================================================================
println("=" ^ 60)
println("Extending N@D2 sweep with N ∈ $N_EXTEND")
println("=" ^ 60)
...
for N in N_EXTEND
    ...
    println(" ESS=$(round(Int, rg.min_ess)) / med=$(round(Int, rg.median_ess))  " *
            "t/it=$(round(rg.t_per_iter_ms, digits=2))ms  total=$(round(rg.total_time_s, digits=1))s")
...
# ==================================================================
# Re-render using full CSV
# ==================================================================
println("\nRendering figure with extended sweep...")
...
safe_pos(xs) = [x <= 0 || isnan(x) ? 1.0 : x for x in xs]
...
df_D = filter(r -> occursin("D@", r.sweep), results)
ax12 = Axis(fig[1, 2]; xlabel="D (feature dim)", ylabel="min ESS",
            title="ESS vs D  (N=50)", xscale=log10, yscale=log10)
...
ax22 = Axis(fig[2, 2]; xlabel="D (feature dim)", ylabel="ms / iter",
            title="Per-iteration cost vs D  (N=50)", xscale=log10, yscale=log10)
...
println("\nExtended results:")
show(stdout, sort(df_N, [:method, :N]); allrows=true, allcols=true)
println()
